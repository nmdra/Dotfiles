...
function formats_fold(width, height, audio_only)
	...
	for _, fmt in ipairs(data[url].formats_unfolded) do
		...
		if not inserted_res[res] or res == unfold_res or (audio_only and res == "audio-only") then
...
function menu_unfold()
	...
	formats_fold(cursor_fmt.width, cursor_fmt.height, is_format_audio_only(cursor_fmt))
	menu_draw()
end
...
function get_ytdl_mpvconf_args()
	...
	if isempty(fmtopt) then
		fmtopt = is_loaded_file_audio_only() and "bestaudio/best" or "bestvideo+bestaudio/best"
	end
...
function reload(path, timepos)
	...
	local success = mp.commandv("loadfile", path, "replace", 0, "start=+" .. timepos) -- mpv >= v0.38.0
	if not success then
